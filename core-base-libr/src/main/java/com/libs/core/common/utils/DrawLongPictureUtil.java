...
public class DrawLongPictureUtil extends LinearLayout {
    ...
    private void initView() {
        ...
        text_time = rootView.findViewById(R.id.text_time);
...
//        imgQrCode = rootView.findViewById(R.id.imgQrCode);
        ...
        heightTop = llTopView.getMeasuredHeight() + PhoneUtil.dp2px(context, 36);
...
//    }
    ...
    private void draw() {
        // 绘制最上面的时间
        String content1 = !TextUtils.isEmpty(shareInfo.getTime()) ? shareInfo.getTime() : "";
        TextPaint contentPaint1 = text_time.getPaint();
        contentPaint1.setColor(text_time.getResources().getColor(R.color.color_999999));
        //contentPaint.setColor();
        //contentPaint.setTextSize();
        StaticLayout staticLayout1 =
                new StaticLayout(content1, contentPaint1, (PhoneUtil.getPhoneWid(context) - picMargin * 2),
                        Layout.Alignment.ALIGN_NORMAL, 1.2F, 0, false);
        heightContent1 = staticLayout1.getHeight();
        ...
        StaticLayout staticLayout =
                new StaticLayout(content, contentPaint, (PhoneUtil.getPhoneWid(context) - picMargin * 2),
                        Layout.Alignment.ALIGN_NORMAL, 1.2F, 0, false);
        heightContent = staticLayout.getHeight() + PhoneUtil.dp2px(context, 40F);



        // 计算出最终生成的长图的高度 = 上、中、图片总高度、下等个个部分加起来
        int allBitmapHeight = heightTop + heightContent + heightBottom + heightContent1;
        // 计算图片的总高度
        ...
        canvas.save();
        canvas.drawBitmap(getLinearLayoutBitmap(timeView, widthBottom, heightTop), PhoneUtil.dp2px(context, 20), (heightTop - PhoneUtil.dp2px(context, 1F)), paint);
        canvas.save();
...
//        staticLayout1.draw(canvas);
        ...
        // 绘制content view
        canvas.translate(PhoneUtil.dp2px(context, 40), heightTop);
        staticLayout1.draw(canvas);
        canvas.translate(PhoneUtil.dp2px(context, -20), PhoneUtil.dp2px(context, 40F));
...
//        }
        ...
        if (imageUrlList != null && imageUrlList.size() > 0) {
            canvas.drawBitmap(getLinearLayoutBitmap(llBottomView, widthBottom, heightBottom), 0,
                    (heightTop + heightContent + heightContent1 + getAllImageHeight() + PhoneUtil.dp2px(context, 16)), paint);
        } else {
            canvas.drawBitmap(getLinearLayoutBitmap(llBottomView, widthBottom, heightBottom), 0,
                    (heightTop + heightContent + heightContent1 + getAllImageHeight()), paint);
        }
        ...
        try {
            ...
            if (listener != null) {
                listener.onSuccess(result, bitmapAll);
                MediaStore.Images.Media.insertImage(context.getContentResolver(), result, "file.getName()", null);
            }
        } catch (IOException e) {
            e.printStackTrace();
            if (listener != null) {
